...
bool compress_data(const uint8_t* input, uint32_t input_len,
    ...
    switch(algorithm) {
        case COMPRESS_DELTA:
            compressed_size = delta_encode(input, input_len, output, *output_len);
            break;

        case COMPRESS_RLE:
            compressed_size = rle_encode(input, input_len, output, *output_len);
            break;
        ...
        case COMPRESS_HUFFMAN:
            ...
            compressed_size = huffman_encode(&huffman_state, input, input_len,
                                             output, *output_len);
            break;

        case COMPRESS_LZ77:
            compressed_size = lz77_encode(input, input_len, output,
                                          *output_len, 4096, 18);
            break;
    ...
    // FIX 3d: return false on overflow
    if(compressed_size == 0 && input_len > 0) {
        FURI_LOG_E(TAG, "Compression output overflow");
        return false;
    }
...
// FIX (note): stub — NOT IMPLEMENTED. Returns false so callers know.
//   Implementing this requires a compressed-stream header that identifies
//   the algorithm; see compress_data() for the encode side.
bool decompress_data(const uint8_t* input, uint32_t input_len,
    ...
    UNUSED(stats);
    *output_len = 0;
    FURI_LOG_E(TAG, "decompress_data: NOT IMPLEMENTED");
    return false;
}

// Delta encoding
uint32_t delta_encode(const uint8_t* input, uint32_t len,
                      uint8_t* output, uint32_t output_max) {  // FIX 3
    if(len == 0) return 0;
...
// Delta decoding
// FIX (critical): added output_max to prevent unbounded write; returns 0 on overflow.
uint32_t delta_decode(const uint8_t* input, uint32_t len,
                      uint8_t* output, uint32_t output_max) {
    if(len == 0 || output_max == 0) return 0;
...
uint32_t delta_encode_16bit(const uint16_t* input, uint32_t len,
                             uint8_t* output, uint32_t output_max) {  // FIX 3
    if(len == 0) return 0;
    ...
    for(uint32_t i = 1; i < len; i++) {
        ...
        if(delta >= -128 && delta <= 127) {
            ...
            if(out_pos + 4 >= output_max) return 0;
            output[out_pos++] = 0x81;
...
// FIX (critical): added output_max (element count) to prevent unbounded write.
uint32_t delta_decode_16bit(const uint8_t* input, uint32_t len,
                             uint16_t* output, uint32_t output_max) {
    if(len < 2 || output_max == 0) return 0;
    ...
    while(in_pos < len) {
        ...
        if(byte == 0x80 && in_pos + 1 < len) {
            ...
            delta = (int32_t)((uint32_t)input[in_pos]   << 24 |
                              (uint32_t)input[in_pos+1] << 16 |
...
// RLE encoding
uint32_t rle_encode(const uint8_t* input, uint32_t len,
                    uint8_t* output, uint32_t output_max) {  // FIX 3
    if(len == 0) return 0;
...
// RLE decoding
// FIX (critical): added output_max; all writes guarded; removed hardcoded
//   COMPRESSION_MAX_BLOCK_SIZE limit — caller owns the buffer.
uint32_t rle_decode(const uint8_t* input, uint32_t len,
                    uint8_t* output, uint32_t output_max) {
    uint32_t in_pos  = 0;
    uint32_t out_pos = 0;
...
// Adaptive RLE
uint32_t rle_encode_adaptive(const uint8_t* input, uint32_t len,
                              uint8_t* output, uint32_t output_max) {  // FIX 3
    ...
    if(run_count > len / 4) {
        return rle_encode(input, len, output, output_max);
    } else {
        return delta_encode(input, len, output, output_max);
    }
}
...
// Huffman encode
uint32_t huffman_encode(const HuffmanState_t* state,
    ...
    for(uint32_t i = 0; i < len; i++) {
        ...
        for(int8_t j = code_len - 1; j >= 0; j--) {
            ...
            if(bit_count == 8) {
                if(out_pos >= output_max) return 0;  // FIX 3: overflow
...
uint32_t lz77_encode(const uint8_t* input, uint32_t len, uint8_t* output,
                     uint32_t output_max,
                     uint16_t window_size, uint16_t lookahead_size) {  // FIX 3
    uint32_t in_pos = 0;
    uint32_t out_pos = 0;
    ...
    while(in_pos < len) {
        ...
        uint16_t max_lookahead = (in_pos + lookahead_size < len) ?
                                 lookahead_size : (uint16_t)(len - in_pos);
        ...
        if(best_length >= 3) {
            if(out_pos + 3 >= output_max) return 0;  // FIX 3
            ...
            if(input[in_pos] == 0x00) {
                if(out_pos + 1 >= output_max) return 0;  // FIX 3
                output[out_pos++] = 0x00;
                output[out_pos++] = 0xFF;  // Escape
            }
            if(out_pos >= output_max) return 0;  // FIX 3
            output[out_pos++] = input[in_pos++];
        }
    }
...
// LZ77 decode
// FIX (critical): added output_max; all writes guarded.
//   Also fixed invalid back-reference (offset==0 or offset>out_pos) UB.
uint32_t lz77_decode(const uint8_t* input, uint32_t len,
                     uint8_t* output, uint32_t output_max) {
    uint32_t in_pos  = 0;
    uint32_t out_pos = 0;
    ...
    while(in_pos < len) {
        ...
        if(byte == 0x00) {
            ...
            if(next == 0xFF) {
                ...
                uint16_t offset = (uint16_t)((next << 8) | input[in_pos++]);
                uint8_t  length = input[in_pos++];
                if(offset == 0 || offset > out_pos) break;
...
uint32_t decompress_pulse_sequence(const uint8_t* input, uint32_t len,
    ...
    uint32_t decompressed = delta_decode_16bit(&input[in_pos], len - in_pos, widths, count);
    in_pos += decompressed;
...
CompressionAlgorithm_t compression_select_algorithm(const uint8_t* sample_data,
    ...
    // Test delta
    test_len = delta_encode(sample_data, sample_len, test_output, sizeof(test_output));
    float delta_ratio = (float)sample_len / (float)test_len;
    
    // Test RLE
    test_len = rle_encode(sample_data, sample_len, test_output, sizeof(test_output));
...
float compression_estimate_ratio(const uint8_t* data, uint32_t len,
    ...
    switch(algorithm) {
        case COMPRESS_DELTA:
            test_len = delta_encode(data, len > 256 ? 256 : len, test_output, sizeof(test_output));
            break;
        case COMPRESS_RLE:
            test_len = rle_encode(data, len > 256 ? 256 : len, test_output, sizeof(test_output));
            break;
        default:
            return 1.0f;
    }
