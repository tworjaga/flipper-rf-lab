...
#define MAX_STACK_DEPTH         4096            // 4KB per task (default)
#define RF_CAPTURE_STACK_SIZE   (4  * 1024)     // 4 KB — lean ISR-driven loop
#define UI_UPDATE_STACK_SIZE    (4  * 1024)     // 4 KB — display only
#define ANALYSIS_STACK_SIZE     (8  * 1024)     // 8 KB — compression + clustering
...
typedef struct {
    ...
    int8_t  tx_power_dbm;   // TX power in dBm (-20 to +10)
...
typedef struct {
    ...
    int16_t  rssi_dbm;              // RSSI at capture time (dBm, negative)
...
typedef struct {
    ...
    uint16_t battery_mv;          // Battery voltage in millivolts (e.g. 3300 = 3.3V)
...
// SHUTDOWN FLAG
// ============================================================================
// FIX (note): volatile is correct for single-core Cortex-M4 (prevents compiler
// elision). If ever ported to a dual-core target use an atomic or event-group
// instead — volatile alone does not provide cross-core memory ordering.
