...
KMeansResult_t clustering_kmeans(Dataset_t* data, uint8_t k) {
    ...
    // Seed centroids evenly across data range — avoids degenerate first-k init (FIX 11)
    fixed_t x_min = data->points[0].x, x_max = data->points[0].x;
    for(uint16_t i = 1; i < data->count; i++) {
        if(data->points[i].x < x_min) x_min = data->points[i].x;
        if(data->points[i].x > x_max) x_max = data->points[i].x;
    }
    fixed_t step = (k > 1) ? (x_max - x_min) / (k - 1) : 0;
    for(uint8_t i = 0; i < k; i++) {
        result.centroids[i].x = x_min + step * i;
        // FIX (minor): guard index when data->count is not divisible by k
        uint16_t pt_idx = (k == 1) ? 0 : (uint16_t)(i * (data->count / k));
        if(pt_idx >= data->count) pt_idx = data->count - 1;
        result.centroids[i].y = data->points[pt_idx].y;
...
void clustering_kmeans_iterative(KMeansResult_t* result, Dataset_t* data) {
...
// Assign points to nearest centroids
bool clustering_kmeans_assign_points(KMeansResult_t* result, Dataset_t* data) {
    bool changed = false;
    ...
    for(uint16_t i = 0; i < data->count; i++) {
        ...
        // FIX (critical): removed const-cast UB. Function now takes Dataset_t*
        //   (non-const) so assignment writes are legal.
...
void clustering_kmeans_update_centroids(KMeansResult_t* result, Dataset_t* data) {
