...
// FIX (critical): 999 × ~70 bytes = ~68 KB BSS; reduced to a realistic limit.
...
typedef struct {
    ...
    uint32_t start_tick;        // FIX: furi_get_tick() at session open (for duration calc)
    uint32_t num_frames;
...
// Magic + version allows safe schema-change detection on load.
#define SESSION_INDEX_MAGIC   0x52464C42U   // 'RFLB'
#define SESSION_INDEX_VERSION 2

typedef struct {
    uint32_t magic;             // Must equal SESSION_INDEX_MAGIC
    uint16_t version;           // Must equal SESSION_INDEX_VERSION
...
// FIX (major): open_file with write=true no longer always truncates.
//   Use sd_manager_open_file_append() for log files.
